...
subroutine M1_updateeas
  ...
  real*8 :: xrho,xtemp,xye,xeta,xxp,xxn,xmu_mu,xxmu
  ...
  if (M1_testcase_number.eq.0 .or.M1_testcase_number.eq.1) then
   ...
   !     write(*,*) nulibtable_logtemp_max
        ...
        xrho = rho(k)/rho_gf
        xtemp = temp(k)
        xye = ye(k)+ymu(k) ! lepton fraction
        ...
        if (.true.) then
           ...
           call nuc_eos_full(xrho,xtemp,xye,eosdummy(1),eosdummy(2),eosdummy(3), &
                ...
                eosdummy(13),elechem(k),eosdummy(15),eosdummy(16),eosdummy(17),xmu_mu,xxmu, &
           ...
           do j=1,number_groups
			...
			if (number_species_to_evolve .GT. 3) then
              blackbody_spectra(4,j) = clite*(nulibtable_energies(j)/nulib_energy_gf)**3* &
                   (mev_to_erg/(2.0d0*pi*hbarc_mevcm)**3)/(1.0d0+exp(energy_x-xeta))
              blackbody_spectra(5,j) = clite*(nulibtable_energies(j)/nulib_energy_gf)**3* &
                   (mev_to_erg/(2.0d0*pi*hbarc_mevcm)**3)/(1.0d0+exp(energy_x+xeta))
              blackbody_spectra(6,j) = clite*(nulibtable_energies(j)/nulib_energy_gf)**3* &
                   (mev_to_erg/(2.0d0*pi*hbarc_mevcm)**3)/(1.0d0+exp(energy_x))
			endif 
           ...
           eas(k,3,:,1) = 4.0d0*(blackbody_spectra(3,:)*tempspectrum(3,:,2)* &
                nulibtable_ewidths(:)/nulib_opacity_gf)*nulib_emissivity_gf
			if (number_species_to_evolve .GT. 3) then
			 eas(k,4,:,1) = (blackbody_spectra(4,:)*tempspectrum(4,:,2)* &
                nulibtable_ewidths(:)/nulib_opacity_gf)*nulib_emissivity_gf
           eas(k,5,:,1) = (blackbody_spectra(5,:)*tempspectrum(5,:,2)* &
                nulibtable_ewidths(:)/nulib_opacity_gf)*nulib_emissivity_gf
           eas(k,6,:,1) = 4.0d0*(blackbody_spectra(6,:)*tempspectrum(6,:,2)* &
                nulibtable_ewidths(:)/nulib_opacity_gf)*nulib_emissivity_gf
			endif
        else
           eas(k,:,:,:) = tempspectrum(:,:,:)
