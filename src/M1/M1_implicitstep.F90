...
subroutine M1_implicitstep(dts,implicit_factor)
  ...
  use GR1D_module
  use GR1D_module, only : loop_on_specie
  ...
  integer :: i,j,k,ii,jj,count,j_prime,shifty
  integer :: location(1)
  integer :: info
...
!~   write(*,*) loop_on_specie
  
  do k=ghosts1+1,M1_imaxradii
     do shifty=1,number_species_to_evolve
        i=loop_on_specie(shifty)
        if (GR) then
        ...
        if ((include_epannihil_kernels).or.include_Ielectron_imp &
...
!~                  write(*,*) "anue",dts
              ...
              do j=1,number_groups
                 ...
                 local_dMbardF(2,1) = local_dMbardF(1,2)
...
!~            endif
           ...
           if (count.gt.100.and.maxval(abs(RF)).lt.1.0d-5) then
              ...
              write(*,*) "warning, no tolerance after 100 iterations", k,i
              stop
        ...
        do j=1,number_groups
           ...
           if (include_epannihil_kernels.or.include_bremsstrahlung_kernels) then
			  ...
			  !if (i .eq. 1) write(*,*) epannihil_sourceterm(k,i,j,1)
           endif
